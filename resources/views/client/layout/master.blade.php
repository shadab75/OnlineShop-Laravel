...
<body>
<div class="wrapper-wide" style="background-color: #FFFFFF">
    <div id="header">
        <!-- Top Bar Start-->
                    ...
                    <div class="col-table-cell col-lg-3 col-md-3 col-sm-6 col-xs-12">
                        <div id="cart">
                            <button type="button" data-toggle="dropdown" data-loading-text="Loading..." class="heading dropdown-toggle">
                                ...
                                <span id="cart-total">
                                <span id="total-items">{{\App\Models\Cart::totalItems()}}</span>آیتم -
                                <span id="total-amount">  {{\App\Models\Cart::totalAmount()}} </span>تومان
                          </span> </button>
                            <ul class="dropdown-menu">
                                <li>
                                    ...
                                    <div>
                                        <table class="table table-bordered">
                                            ...
                                            <tr>
                                                ...
                                                <td class="text-right"> <span id="total-1">{{\App\Models\Cart::totalAmount()}} تومان</span></td>
                                            </tr>
                                            <tr >
                                                <td class="text-right"><strong>قابل پرداخت</strong></td>
                                                <td class="text-right"><span id="total-2">{{\App\Models\Cart::totalAmount()}} تومان</span></td>
                                            </tr>
                                            </tbody>
                                        </table>
    ...
    <script>
        ...
        $.ajax({
            ...
            success:function (data){
                ...
                $('#total-1').text(data.cart.total_amount)
                $('#total-2').text(data.cart.total_amount)
                if (!$('#cart-row-' + productId).length)
                {
                    ...
                    $('#cart-table-body:last-child').append(
                        ...
                        +'<td class="text-left"><a href="product.html">'+product.name+'</a></td>'
                        +'<td class="text-right">x'+productQty+'</td>'
                        +' <td class="text-right">'+product.cost_with_discount+'</td>تومان'
                        +' <td class="text-center"><button class="btn btn-danger btn-xs remove" title="حذف" onClick="removeFromCart('+product.id+')" type="button"><i class="fa fa-times"></i></button></td>'
                        +'</tr>'
...

        {
            $.ajax({
                ...
                success:function (data){
                    ...
                    $('#trcreate-'+productId).remove()
                    $('#trindex-'+productId).remove()
        ...
        function updateCart(productId)
        {
            ...
            var inputquantity = $('#input-quantity-'+productId).val()
            ...
            $.ajax({
                ...
                success:function (data){
                $('#total-amount-'+productId).text(data.cart.total_amount)
                $('#input-quantity-'+productId).val(data.cart[productId]['quantity'])
